     ...
     LEFT JOIN dbo.Data_Hero b ON a.championId = b.championId " + whereStart + where, null);
        ...
        public int AddBattleList(string json)
        {
            ...
            if (count == 0)
            {
                ...
                float KDA = float.Parse(((participantsItem.stats.kills + participantsItem.stats.assists) /
                                         float.Parse((participantsItem.stats.deaths).ToString())).ToString("#0.00"));
                ...
                int result = DapperHelper<int>.Execute(insertSQL,
                    new
                    {
                        ...
                        KDA = KDA,
                        gameEndedInEarlySurrender = participantsItem.stats.gameEndedInEarlySurrender == "true"?1 : 0
                    });
